...
DRsimPrimaryGeneratorAction::DRsimPrimaryGeneratorAction(G4int seed, G4bool useHepMC, G4bool useCorsika, G4bool useCalib, G4bool useGPS)
...
void DRsimPrimaryGeneratorAction::GeneratePrimaries(G4Event* event) {
  ...
  if (fUseCorsika) {
    G4AutoLock lock(&DRsimPrimaryGeneratorMutex);
    DRsimRunAction::sCorFlatG4Reader->GeneratePrimaryVertex(event);
    sIdxEvt = sNumEvt;
    sNumEvt++;
